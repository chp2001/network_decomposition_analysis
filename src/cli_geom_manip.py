...
from cli_access_utils import get_vpu_wbids
from cli_access_utils import all_wbids, check_wbids_valid, vpu_list, vpu_stats
from cli_multiprocess import WorkerManager
from cli_debug_utils import MemoryMonitor, debug_geoplot_geom
...
def data_bisect(
    ...
    repl = lambda i, ax, v: tuple(
        v if ((~(k % 2) ^ ax) & (k // 2 == i)) else l for k, l in enumerate(bounds)
    )
...
def geometric_partition(
    data: list, partition_size: int, bounds: tuple, path=""
) -> list[list, tuple, str, tuple, tuple]:
    assert (
        bounds[0] < bounds[2]
    ), f"Bounds are not valid: {bounds}, {bounds[0]} < {bounds[2]}"
    assert (
        bounds[1] < bounds[3]
    ), f"Bounds are not valid: {bounds}, {bounds[1]} < {bounds[3]}"
    ...
    try:
        ...
        if isinstance(total_data, list):
            ...
            if isinstance(total_data[0], tuple):
                print(
                    f"Total data item 0 types: {set([str(type(d)) for d in total_data[0]])}"
                )
...
def collect_step_mp(parts) -> list[list, tuple, str]:
    ...
    args = [
        {"part0": parts[i], "part1": parts[i + 1]}
        for i in range(0, len(parts) - start_len % 2, 2)
    ]
    ...
    try:
        ...
        if len(skips) > 0 and len(results) > 0:
            final = results[-1]
            results = results[:-1]
            merged = mp_collect_partitions(
                {"part0": final, "part1": skips[0], "override": True}
            )
        ...
        assert (
            final_len < start_len
        ), f"Final length is not less than start length: {final_len} >= {start_len}"
        return results
...
def mp_full_partition_1(
    geoms: list, partition_size: int, bounds: tuple, profile_dir: Path
):
        ...
        for i, subquad in enumerate(quads):
            ...
            for j, _quad in enumerate(subquad):
                ...
                if len(_quad) >= len(quad):
                    ...
                    try:
                        return recursor(
                            _quad, bounds_list[2 * i + j], paths[2 * i + j], depth + 1
                        )
                        ...
                        raise Exception(e)
                rs_.append(
                    recursor(_quad, bounds_list[2 * i + j], paths[2 * i + j], depth + 1)
                )
            result.append(rs_)
        return result
...
def heavy_union(
    geoms: list, partition_size: int = 5000, mp_threshold: int = 10000
) -> Geometry:
    ...
    if do_mp:
        ...
        print(
            f"Collected {num_parts} partitions in {time.time() - collect_time} seconds"
        )
        print(f"Finalizing")
        return parts[0][0]
    else:
...
if __name__ == "__main__":
    ...
    if do_vpu_list:
        ...
        # get the smallest vpuid, ignoring "None"
        smallest = min(
            [k for k in stats["vpu_totals"] if k is not None],
            key=lambda k: stats["vpu_totals"][k],
        )
        ...
        start_time = time.time()
        merged_geom = heavy_union(
            [geoms_dict[w] for w in valid_wbids], partition_size=500
        )
    ...
    if do_all_wbids:
        with MemoryMonitor() as mem:
            ...
            geoms_dict = gpkg_utils.get_geom_from_wbids_map(all_wbs)
            print(
                f"Got {len(geoms_dict)} geometries in {time.time() - start_time} seconds"
            )
            start_time = time.time()
            merged_geom = heavy_union(
                [geoms_dict[w] for w in all_wbs], partition_size=500
            )
            print(f"Union took {time.time() - start_time} seconds")
    ...
    if do_less_than_all_wbids:
        with MemoryMonitor() as mem:
            ...
            half_wbids = [
                wb for wbgroup in half_wbid_groups for wb in wbgroup if wb in all_wbs
            ]
            ...
            start_time = time.time()
            merged_geom = heavy_union(
                [geoms_dict[w] for w in half_wbids], partition_size=500
            )
            print(f"Union took {time.time() - start_time} seconds")
