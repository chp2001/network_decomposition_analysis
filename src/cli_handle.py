...
def _mp_create_wbid_map(ctx):
    ...
    print(
        f"Processed {chunks_done.value} chunks, {float(items_done.value)/total_items.value*100:.2f}%",
        end="\r",
    )
    return partial_map
...
def _mp_merge_wbid_maps(ctx):
    ...
    print(
        f"Merged {chunks_done.value} chunks, {float(items_done.value)/total_items.value*100:.2f}%",
        end="\r",
    )
    return map1
...
def _mp_subset_dissolve(ctx):
    ...
    if chunks_done.value % 100 == 0:
        print(
            f"Dissolved {chunks_done.value} chunks, {float(items_done.value)/total_items.value*100:.2f}%",
            end="\r",
        )
    return union, ctx[1]
...
def gpkg_tnx_interface():
    ...
    # exit()
    graph = graph_utils.generate_subgraph(
        graph, set([v.index for v in graph.vs if len(v["name"]) > 0]), wbids
    )
    print(f"Decomposing")
    ...
    lastmost = lambda s: s.vs.select(lambda v: len(v.successors()) == 0)
    lastsmosts = {i: lastmost(s) for i, s in enumerate(subgraphs_)}
    all_lasts = []
    _i = 0
    for i, s in enumerate(subgraphs_):
        if not sgraph_has_tnx(s):
            _i += 1
            # print(f"Subgraph {i} has no tnx")
            for v in lastsmosts[i]:
                # print(f"Lastmost vertex of {i}: {v['name']}")
                all_lasts.append(v["name"])
    print(f"Got {len(all_lasts)} lastmost vertices")
    print(f"There were {_i} subgraphs without tnx")
    if bool(input("Print to file? ")):
        with open(File_Paths.root_output_dir() / "lastmost.txt", "w") as f:
            f.write("\n".join(all_lasts))
    exit()
    ...
    subgraphs = [
        s for s in subgraphs if len(s.vs.select(lambda v: v["name"] in wbids)) > 0
    ]
    ...
    if _check_pickle("wb_subset_indices"):
        ...
        with Pool(
            initializer=_mp_init_shared_mem, initargs=(num_finished, n_finished, total)
        ) as p:
            print(f"Starting pool with {p._processes} processes")
            try:
                pool_result = p.imap_unordered(
                    _mp_create_wbid_map,
                    [(i, subgraph) for i, subgraph in enumerate(subgraphs)],
                    chunksize=10,
                )
                ...
                while n_ > 1:
                    ...
                    print(
                        f"Reduced to {n_} chunks. {percent(len(subgraphs) - n_, len(subgraphs))}",
                        end="\r",
                    )
    ...
    for i, v in sort_order:
        ...
        if complete_ > 0:
            if (
                prog_(complete_, len(geoms)) - last_progress > 3
                or time.time() - last_progress_time > 10
            ):
                ...
                _send_pickle(
                    _savedata, f"geom_final_partitions_{len(subgraphs)}_{len(geoms)}"
                )
        ...
        print(
            f"Adding {unique_subset_name} with {v} vertices and {len(geom_)} geometries"
        )
    ...
    gdf_initial = GeoDataFrame(
        {"geometry": subset_geoms, "size": size_list},
        crs="EPSG:5070",
        geometry="geometry",
    )
    ...
    _insert_data_gpkg(
        File_Paths.root_output_dir() / filename, gdf_initial, "flow_subgraphs"
    )
    file_end = time.time()

    print(
        f"Created, cleaned, and inserted data into gpkg in {file_end - file_start} seconds"
    )
...
def _mp_find_bounds(ctx):
    ...
    if data["num_finished"].value % 100 == 0:
        print(
            f"Processed {data['num_finished'].value}/{data['total'].value} geometries",
            end="\r",
        )
...
def _data_bisect(
    data: list, bounds: tuple, x=None, y=None, path=""
) -> list[list, tuple]:
...
def _geometric_partition(
    data: list, partition_size: int, bounds: tuple, _depth=0, path=""
) -> list[list, tuple]:
    ...
    if _depth < 1:
        ...
        for p, b, path_ in parts:
            if len(p) > partition_size:
                _parts.extend(
                    _geometric_partition(p, partition_size, b, _depth + 1, path=path_)
                )
            else:
...
def _mp_partition(ctx):
    ...
    if log2(data["num_finished"].value).is_integer():
        print(
            f"Partitioned {data['num_finished'].value} chunks, avg size: {sum([len(p) for p, b, path in parts])/len(parts)}",
            end="\n",
        )
    return parts
...
def _geom_unary_merge_union(geoms: list, name: str = "") -> Geometry:
    ...
    if not _check_pickle(f"geom_final_partitions{postfix}"):
            ...
            for i in range(4):
                ...
                for p, b, path in partitions:
                    if len(p) > desired_chunksize:
                        new_parts.extend(
                            _geometric_partition(p, desired_chunksize, b, path=path)
                        )
                    else:
        ...
        final_partitions = [
            (p, b, path) for p, b, path in partitions if len(p) <= desired_chunksize
        ]
        partitions = [
            (p, b, path) for p, b, path in partitions if len(p) > desired_chunksize
        ]
            ...
            with Pool(initializer=_mp_init_shared_mem_generic, initargs=(data,)) as p:
                ...
                try:
                    ...
                    while len(partitions) > 0:
                        ...
                        contexts = [
                            (p, desired_chunksize, b, path) for p, b, path in partitions
                        ]
                        pool_result = p.imap_unordered(
                            _mp_partition, contexts, chunksize=5
                        )
                        pool_result = list(pool_result)
                        partitions = []
                        ...
                        if len(partitions) > 0:
                            print(
                                f"Cycle {_cycle}: Partitions: {len(partitions)}, MaxSize: {max([len(p) for p, _, _ in partitions])}",
                                end="\n",
                            )
    ...
    print(
        f"Average size: {sum([len(p) for p, _, _ in final_partitions])/len(final_partitions)}"
    )
    ...
    if diagnostic_needed:
        ...
        plt.show()
        plt.savefig(
            File_Paths.root_output_dir() / "final_partitions" + postfix + ".png"
        )
        return
    ...
    if needed_unions != len(final_partitions):
        ...
        print(
            f"Original tree size: {original_tree_size}, Needed tree size: {needed_tree_size}"
        )
    ...
    for i in range(len(final_partitions)):
        ...
        # exit()
        with Pool(
            initializer=_mp_init_shared_mem,
            initargs=(chunks_done, items_done, total_items),
        ) as p:
            print(f"Starting pool with {p._processes} processes")
            try:
                print(f"Unioning {pairs}")
                chunks = [
                    [[unions[p[0]], unions[p[1]]], k] for k, p in enumerate(pairs)
                ]
